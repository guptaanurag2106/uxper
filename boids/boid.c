...
void update_boid(Boid *boids, size_t index, size_t count, int screen_width,
    ...
    for (size_t j = 0; j < count; j++) {
        ...
        float dx = boid->x - b2->x;
        float dy = boid->y - b2->y;
        float dist2 = dx * dx + dy * dy + epsilon;

        if (dist2 > BOID_NEIGHBOUR_DIST * BOID_NEIGHBOUR_DIST) continue;

        float rel_angle = atan2f(boid->y - b2->y, b2->x - boid->x);
        if (rel_angle < 0) rel_angle += 2 * PI;

        float angle_diff = wrap_float(rel_angle - boid->theta, -PI, PI);
        if (fabs(angle_diff) > BOID_FOV) continue;

        separation_force_x += dx / dist2;
        separation_force_y += dy / dist2;
    ...
    if (neighbours > 0) {
        ...
        float sdiff = separation_theta - boid->theta;

        float avg_theta = atan2f(sin_sum, cos_sum);
        if (avg_theta < 0) avg_theta += 2 * PI;
        float adiff = avg_theta - boid->theta;

        float centre_theta =
            atan2(y_sum / neighbours - boid->y, x_sum / neighbours - boid->x);
        if (centre_theta < 0) centre_theta += 2 * PI;
        float cdiff = centre_theta - boid->theta;
        boid->theta =
            wrap_float(boid->theta + dt * (sdiff * SEPARATION_STRENGTH +
                                           adiff * ALIGNMENT_STRENGTH +
