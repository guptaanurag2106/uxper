...
var bg = make([]byte, ScreenWidth*ScreenHeight)
...
func renderGame() {
	...
	copy(gameState.image, bg)

	// Left, Right paddle
	lPaddlePos := int(gameState.lPaddlePos)
	rPaddlePos := int(gameState.rPaddlePos)
	for i := -PaddleHeight / 2; i < PaddleHeight/2; i++ {
		for j := PaddleWidth; j < 2*PaddleWidth; j++ {
			gameState.image[(j)+(lPaddlePos+i)*ScreenWidth] = 0xFF
			gameState.image[(ScreenWidth-j)+(rPaddlePos+i)*ScreenWidth] = 0xFF
		}
	}
	...
	for i := -BallRadius; i < BallRadius; i++ {
		for j := -BallRadius; j < BallRadius; j++ {
			gameState.image[(ballX+j)+(ballY+i)*ScreenWidth] = 0xFF
		}
	}
}
...
func main() {
	...
	if texture == nil {
		fmt.Fprintf(os.Stderr, "SDL_CreateTexture failed: %s\n", C.GoString(C.SDL_GetError()))
		os.Exit(1)
	}
	...
	for i := 0; i < ScreenHeight-DividerWidth; i += DividerWidth {
		if divider {
			for j := i; j < i+DividerWidth; j++ {
				for w := -DividerWidth / 2; w < DividerWidth/2; w++ {
					bg[(ScreenWidth/2+w)+(j)*ScreenWidth] = 0b10010010
				}
			}
		}
		divider = !divider
	...
	for !quit {
		...
		if gameState.screen == ScreenGame {
			...
			if C.SDL_LockTexture(texture, nil, &pixels, &pitch) == 0 {
				// FIX: assuming pitch == ScreenWidth
				gameState.image = (*[1 << 30]byte)(pixels)[:ScreenWidth*ScreenHeight]
				renderGame()
				gameState.image = nil
		...
		// 	texture,
		// 	nil,
		// 	unsafe.Pointer(&gameState.image[0]),
		// 	ScreenWidth,
		// )
